    ...
    <div className="max-w-3xl space-y-10">
      <header className="space-y-4">
        ...
        <h1 className="text-4xl font-bold tracking-tight sm:text-5xl">
          Configure VaultScope subdomains
        </h1>
        <p className="text-lg text-muted-foreground">
          VaultScope subdomains provide quick hostnames under{' '}
          <code>vaultscope.dev</code> or <code>cptcr.dev</code>. Point them at
          panel allocations to share friendly URLs with your community.
        </p>
      </header>

      <section className="space-y-6">
        <h2 className="text-2xl font-semibold">
          1. Open the subdomain manager
        </h2>
        <p className="text-muted-foreground">
          Inside your server view, click <em>More</em> → <em>Subdomains</em>.
          Each server can own one active subdomain at a time.
        </p>
        <DocsScreenshot
          image={subdomainMenu}
      ...
      <section className="space-y-6">
        <h2 className="text-2xl font-semibold">
          2. Choose the hostname and allocation
        </h2>
        <p className="text-muted-foreground">
          Pick a unique subdomain, select the allocation (internal port) that
          serves your workload and confirm the assignment. Changes propagate
          within minutes.
        </p>
        <DocsScreenshot
        ...
        <ul className="pl-6 space-y-2 list-disc text-muted-foreground">
          <li>
            Remove an existing subdomain before claiming a new one—one hostname
            per server.
          </li>
          <li>
            Tag subdomains with environment names (e.g., <code>prod-lobby</code>
            , <code>staging-api</code>) for clarity.
          </li>
          <li>
            Need extra ports? Request them under the <em>Network</em> tab before
            assigning the subdomain.
          </li>
        </ul>
      </section>
      ...
      <section className="space-y-6">
        <h2 className="text-2xl font-semibold">
          3. Bring your own SSL via reverse proxy
        </h2>
        <p className="text-muted-foreground">
          Subdomains are HTTP-only pointers. For HTTPS certificates or custom
          domains, route traffic through a reverse proxy instead.
        </p>
