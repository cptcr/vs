    ...
    <div className="max-w-3xl space-y-10">
      <header className="space-y-4">
        ...
        <h1 className="text-4xl font-bold tracking-tight sm:text-5xl">
          Quickstart your first VaultScope server
        </h1>
        <p className="text-lg text-muted-foreground">
          Follow the same provisioning path our support team shares with new
          customers. These steps turn a brand-new account into an active server
          with control panel access in under five minutes.
        </p>
      </header>

      <section className="space-y-6">
        <h2 className="text-2xl font-semibold">
          1. Create your billing profile
        </h2>
        <p className="text-muted-foreground">
          ...
          and create a billing account. This unlocks the storefront, payment
          history and automated invoices.
        </p>
        <DocsScreenshot
          image={createAccount}
        ...
        <p className="px-4 py-3 text-sm leading-relaxed border rounded-lg border-border/60 bg-muted/10 text-muted-foreground">
          Verification emails are only sent from{' '}
          <span className="font-semibold text-foreground">
            robot@vaultscope.dev
          </span>
          . Add the address to your allowlist if you use aggressive spam
          filtering.
        </p>
      </section>
      ...
      <section className="space-y-6">
        <h2 className="text-2xl font-semibold">
          2. Approve the verification email
        </h2>
        <p className="text-muted-foreground">
          Open the verification email in your inbox and click the confirmation
          link. If the message does not arrive within two minutes, check your
          spam or updates folder before requesting a resend.
        </p>
        <DocsScreenshot
      ...
      <section className="space-y-6">
        <h2 className="text-2xl font-semibold">
          3. Purchase your first server
        </h2>
        <p className="text-muted-foreground">
          After verification, sign in to the billing area and choose the game or
          workload template that matches your project. The storefront walks you
          through hardware selection and location preferences.
        </p>
        <DocsScreenshot
        ...
        />
        <p className="text-muted-foreground">
          Successful purchases display an on-screen confirmation and immediately
          queue the server for deployment.
        </p>
        <DocsScreenshot
      ...
      <section className="space-y-6">
        ...
        <p className="text-muted-foreground">
          A password reset email arrives from{' '}
          <span className="font-semibold text-foreground">
            robot@vaultscope.dev
          </span>{' '}
          as soon as the server finishes installing. Use it to set a control
          panel password that is different from your billing credentials.
        </p>
        <DocsScreenshot
      ...
      <section className="space-y-6">
        <h2 className="text-2xl font-semibold">
          5. Sign in at panel.vaultscope.dev
        </h2>
        <p className="text-muted-foreground">
          Head to{' '}
          <a
          ...
          and log in with the billing email and the password you just created.
          Your newly provisioned server appears on the dashboard with live
